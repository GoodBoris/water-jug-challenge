...
export class ParallelCalculateSolutionsStrategy extends CalculateSolutionsStrategyAbstract {
  ...
  public async calculateSolutions(firstJugCapacity: number, secondJugCapacity: number, targetCapacity: number): Promise<[ISolutionStep[], ISolutionStep[]]> {
    const fromFirstToSecondJugSolution = this.pool.pourWater(firstJugCapacity, secondJugCapacity, targetCapacity);
    const fromSecondToFirstJugSolution = this.pool.pourWater(secondJugCapacity, firstJugCapacity, targetCapacity);
    return Promise.all([fromFirstToSecondJugSolution, fromSecondToFirstJugSolution])

...
export class WaterJugSolutionService implements IWaterJugSolutionService {
  constructor(
    @Inject('CALCULATION_STRATEGY')
    private readonly calculateSolutionsStrategy: ICalculateSolutionsStrategy
  ) {}
  ...
  async calculateSteps({
    ...
    if (
      ...
      return {
        resolution: SolutionResult.NoSolution,
        direction: PourDirection.NoDirection,
        steps: [],
      };
    }
    const [solution1, solution2] =
      await this.calculateSolutionsStrategy.calculateSolutions(
        firstJugCapacity,
        secondJugCapacity,
        targetCapacity
      );
    const bestResult = WaterJugSolutionService.getBestResult(
      solution1,
      solution2
    );
  ...
  private static readonly getBestResult = (
    first: ISolutionStep[],
    second: ISolutionStep[]
  ): { direction: PourDirection; solution: ISolutionStep[] } =>
